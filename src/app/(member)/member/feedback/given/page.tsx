...
export default function MemberFeedbackGivenPage() {
    ...
    // Fetch feedback entries where this employee is the author (given feedback)
    const fetchGivenFeedback = useCallback(async () => {
        ...
        setLoading(true)
        try {
            const response = await fetch(`${API_BASE_URL}/feedback-entries?authorId=${encodeURIComponent(employeeId)}`, {
                method: 'GET',
                headers: {
    ...
    }, [isDataLoading, employeeId, fetchGivenFeedback])
        ...
        return (
            <div className="animate-in fade-in duration-500 slide-in-from-bottom-4 space-y-6">
                <div>
                    ...
                    <p className="text-slate-500">View feedback you have given</p>
                </div>
