...
export function AssetList() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setResources(items.filter((item) => !['book', 'expense', 'reimbursement', 'subscription'].includes(resourceType(item))))
            ...
            <Card className="overflow-hidden rounded-3xl border-slate-100 bg-white shadow-premium">
                <CardContent className="p-0">
                    <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 p-5 sm:p-8">
                        ...
                        <Filter value={typeFilter} setValue={setTypeFilter} label="All types" options={(RESOURCE_TYPES as readonly string[]).filter((item) => !['book', 'expense', 'reimbursement', 'subscription'].includes(item))} />
