...
import { buildLibraryBookPayload, type LibraryBookValues } from '../library'
...
export function LibraryBookForm() {
    ...
    const submit = async (event: React.FormEvent) => {
        ...
        if (!apiAccessToken) {
            toast('You must be signed in to add a book.', 'error')
            return
        }
            ...
            <CardContent className="p-8">
                <form className="space-y-8" onSubmit={submit}>
                    ...
                    <div className="flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-6">
                        <Button type="button" variant="outline" className="rounded-xl" onClick={() => router.push('/resources/library')} disabled={saving}>Cancel</Button>
                        <Button type="submit" className="rounded-xl bg-blue-600 text-white hover:bg-blue-700" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <BookOpen />}Add book</Button>
                    </div>
                </form>
            </CardContent>
        </Card>
