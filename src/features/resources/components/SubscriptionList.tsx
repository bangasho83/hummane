...
export function SubscriptionList() {
    ...
    const [view, setView] = useState<'employee' | 'subscription'>('employee')
    ...
    const groupedSubscriptions = useMemo(() => {
        const groups = new Map<string, { subscription: Resource; count: number; seats: number; totalCost: number; assignees: string[] }>()
        filtered.forEach((subscription) => {
            const name = resourceName(subscription)
            const vendorName = resourceVendor(subscription, vendorNames)
            const key = `${name.toLowerCase()}::${vendorName.toLowerCase()}`
            const details = resourceDetails(subscription)
            const assignee = assignmentEmployeeName(resourceAssignment(subscription), employeeNames) || 'Company'
            const current = groups.get(key)
            if (current) {
                current.count += 1
                current.seats += Number(details.numberOfSeats) || 0
                current.totalCost += resourceCost(subscription) || 0
                if (!current.assignees.includes(assignee)) current.assignees.push(assignee)
            } else {
                groups.set(key, { subscription, count: 1, seats: Number(details.numberOfSeats) || 0, totalCost: resourceCost(subscription) || 0, assignees: [assignee] })
            }
        })
        return [...groups.values()].sort((a, b) => b.totalCost - a.totalCost || resourceName(a.subscription).localeCompare(resourceName(b.subscription)))
    }, [employeeNames, filtered, vendorNames])
