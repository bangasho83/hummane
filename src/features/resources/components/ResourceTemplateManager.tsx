...
import { Pencil, Plus, Save, Archive, Loader2, RotateCcw } from 'lucide-react'
...
const priceInputValue = (amount: number | null | undefined) => amount == null ? '' : String(Number(amount))
...
export function ResourceTemplateManager() {
    ...
    const edit = (template: ResourceTemplate) => {
        ...
        setForm({
            ...
            defaultCostAmount: priceInputValue(template.defaultCostAmount),
    ...
    const unarchive = async (template: ResourceTemplate) => {
        if (!apiAccessToken) return
        try {
            const restored = await updateResourceTemplateApi(template.id, { isActive: true }, apiAccessToken)
            setTemplates((current) => current.map((item) => item.id === restored.id ? restored : item))
            toast('Template restored.', 'success')
        } catch (error) { toast(error instanceof Error ? error.message : 'Failed to restore template', 'error') }
    }
        ...
        <div className="space-y-6">
            ...
            <Card className="rounded-3xl border-slate-100 bg-white shadow-premium">
                ...
                <CardContent className="p-8 pt-2"><form onSubmit={submit} className="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <Field label="Default price"><Input type="number" min="0" step="any" value={form.defaultCostAmount} onChange={(event) => setField('defaultCostAmount', event.target.value)} placeholder="e.g. 25 or 25.50" disabled={saving} /></Field>
