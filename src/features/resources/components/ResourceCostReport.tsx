...
export function ResourceCostReport() {
    ...
    return <div className="space-y-6">
        ...
        {isHydrating || loading ? <div className="flex justify-center p-20"><Loader2 className="h-8 w-8 animate-spin text-blue-600" /></div> : <>
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Total cost" value={formatSummaryCurrency(aggregate.totalCost, currentCompany?.currency)} /><Metric label="Records" value={String(aggregate.resourceCount)} /><Metric label="Recurring cost" value={formatSummaryCurrency(aggregate.recurringCost, currentCompany?.currency)} /><Metric label="Unsettled" value={formatSummaryCurrency(aggregate.unsettledCost, currentCompany?.currency)} /></div>
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-2"><ReportCard title="Cost by vendor / category"><div className="divide-y divide-slate-100">{grouped.length ? grouped.map(([name, value]) => <div key={name} className="flex items-center justify-between gap-4 p-4"><div><p className="font-bold text-slate-900">{name}</p><p className="text-xs text-slate-400">{value.count} record{value.count === 1 ? '' : 's'}</p></div><p className="font-bold text-slate-800">{formatSummaryCurrency(value.total, currentCompany?.currency)}</p></div>) : <Empty />}</div></ReportCard><ReportCard title="Grouped records in this view"><div className="divide-y divide-slate-100">{groupedRecords.length ? groupedRecords.slice(0, 10).map((group) => <div key={`${group.type}:${group.name}`} className="flex items-center justify-between gap-4 p-4"><div><p className="font-bold text-slate-900">{group.name}</p><p className="text-xs capitalize text-slate-400">{group.type.replace('_', ' ')} · {group.count} record{group.count === 1 ? '' : 's'}</p></div><p className="font-bold text-slate-800">{formatSummaryCurrency(group.total, currentCompany?.currency)}</p></div>) : <Empty />}</div>{groupedRecords.length > 10 && <p className="border-t border-slate-100 p-4 text-center text-xs font-bold uppercase tracking-widest text-slate-400">Showing 10 of {groupedRecords.length} groups</p>}</ReportCard></div>
        </>}
    </div>
}
...
function formatSummaryCurrency(amount: number, currency?: string) { return formatCurrency(Math.round(amount), currency) }
