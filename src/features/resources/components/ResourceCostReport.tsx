    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? <div className="flex justify-center p-20"><Loader2 className="h-8 w-8 animate-spin text-blue-600" /></div> : <>
                ...
                <div className="grid grid-cols-1 gap-6 xl:grid-cols-2"><ReportCard title="Cost by product"><TableHeader productLabel="Product" /><div className="divide-y divide-slate-100">{report.byTemplate.length ? report.byTemplate.map((row) => <div key={`${row.templateId || row.templateName}`} className="flex items-center justify-between gap-4 p-4"><div><p className="font-bold text-slate-900">{row.templateName}</p><p className="text-xs text-slate-400">{row.resourceCount} resource{row.resourceCount === 1 ? '' : 's'}</p></div><p className="font-bold text-slate-800">{formatCurrency(Number(row.totalCost), currency)}</p></div>) : <Empty />}</div></ReportCard><ReportCard title="Cost by employee"><TableHeader productLabel="Employee" /><div className="divide-y divide-slate-100">{report.byEmployee.length ? report.byEmployee.map((row) => <div key={row.employeeId || row.employeeName || 'unassigned'} className="flex items-center justify-between gap-4 p-4"><div><p className="font-bold text-slate-900">{row.employeeName || textValue(row.employeeId) || 'Unassigned'}</p><p className="text-xs text-slate-400">{row.resourceCount} subscription{row.resourceCount === 1 ? '' : 's'}</p></div><p className="font-bold text-slate-800">{formatCurrency(Number(row.totalCost), currency)}</p></div>) : <Empty />}</div></ReportCard></div>
            </>}
        </div>
    )
}
