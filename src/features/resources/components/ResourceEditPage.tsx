...
export function ResourceEditPage({ id, mode }: { id: string; mode: 'resource' | 'book' | 'bill' | 'reimbursement' }) {
    const router = useRouter()
    const { apiAccessToken, isHydrating } = useApp()
    const [resource, setResource] = useState<Resource | null>(null)
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState<string | null>(null)
    const listPath = mode === 'bill' ? '/resources/bills' : mode === 'reimbursement' ? '/resources/reimbursements' : mode === 'book' ? '/resources/books' : '/resources/assets'
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const expected = mode === 'bill' ? resourceType(item) === 'expense' : mode === 'reimbursement' ? resourceType(item) === 'reimbursement' : mode === 'book' ? resourceType(item) === 'book' : !['book', 'expense', 'reimbursement'].includes(resourceType(item))
            if (!expected) throw new Error(mode === 'bill' ? 'Bill not found' : mode === 'book' ? 'Book not found' : 'Resource not found')
            setResource(item)
