...
function ProgressBar({ progress, status }: { progress: number; status: OkrObjectiveStatus }) {
  return <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden"><div className={`h-full rounded-full ${progressClass(status)}`} style={{ width: `${progress}%` }} /></div>
}
...
function ObjectiveForm({
  ...
  return <form className="space-y-4" onSubmit={submit}>
    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      <div><label className="text-xs font-bold text-slate-600">Target value</label><Input required min="0.01" type="number" value={form.targetValue} onChange={e => setForm({ ...form, targetValue: Number(e.target.value) })} className="mt-1" /></div>
...
function ObjectiveCard({ objective, onEdit, onDelete }: { objective: OkrObjective; onEdit: () => void; onDelete?: () => void }) {
  return <Card className="border-slate-200 shadow-sm"><CardContent className="p-4 space-y-3">
    <div className="flex justify-between gap-2"><div className="min-w-0"><p className="font-bold text-slate-900 text-sm leading-tight">{objective.headline || 'Untitled objective'}</p><p className="text-xs text-slate-500 mt-1">Due {new Date(`${objective.dueDate}T00:00:00`).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p></div><div className="flex gap-1"><Button size="icon" variant="ghost" className="h-7 w-7" onClick={onEdit} aria-label="Edit objective"><Edit3 className="h-3.5 w-3.5" /></Button>{onDelete && <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500" onClick={onDelete} aria-label="Delete objective"><Trash2 className="h-3.5 w-3.5" /></Button>}</div></div>
    ...
    <div className="flex items-center justify-between text-xs"><span className="font-semibold text-slate-600">{objective.currentValue} / {objective.targetValue} {objective.unit}</span><span className="font-bold text-blue-600">{objective.progress}%</span></div>
...
export function OkrBoard({ adminView = false }: { adminView?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={Boolean(objectiveDialog)} onOpenChange={open => !open && setObjectiveDialog(null)}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{objectiveDialog?.objective ? 'Edit objective' : 'Add objective'}</DialogTitle></DialogHeader>{objectiveDialog && <ObjectiveForm cycleId={board.cycle.id} objective={objectiveDialog.objective} level={objectiveDialog.level} departmentId={objectiveDialog.departmentId} parentObjectiveId={objectiveDialog.parentObjectiveId} employeeId={objectiveDialog.employeeId} onSave={saveObjective} onCancel={() => setObjectiveDialog(null)} />}</DialogContent></Dialog>
  </div>
}
